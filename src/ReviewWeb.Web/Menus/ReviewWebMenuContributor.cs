...
namespace ReviewWeb.Web.Menus
{
    public class ReviewWebMenuContributor : IMenuContributor
    {
        ...
        {
            var l = context.GetLocalizer<ReviewWebResource>();
            ...
            context.Menu.AddItem(
                new ApplicationMenuItem(
                    ReviewWebMenus.HostDashboard,
                    l["Menu:Dashboard"],
                    "~/HostDashboard",
                    icon: "fa fa-line-chart",
                    order: 2
                ).RequirePermissions(ReviewWebPermissions.Dashboard.Host)
            );

            //TenantDashboard
            context.Menu.AddItem(
                new ApplicationMenuItem(
                    ReviewWebMenus.TenantDashboard,
                    l["Menu:Dashboard"],
                    "~/Dashboard",
                    icon: "fa fa-line-chart",
                    order: 2
                ).RequirePermissions(ReviewWebPermissions.Dashboard.Tenant)
            );
